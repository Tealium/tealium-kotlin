...
class TagManagementDispatcher(
    ...
    internal var webViewLoader = WebViewLoader(
        context,
        urlString,
        afterDispatchSendCallbacks,
        connectivityRetriever = connectivity
    )

    fun callRemoteCommandTags(dispatch: Dispatch) {
        if (!remoteApiEnabled) return

        val remoteCommandScript =
            "utag.track(\"remote_api\", ${JsonUtils.jsonFor(dispatch.payload())})"
    ...
    override fun onEvaluateJavascript(js: String) {
        ...
        val script: String =
            if (js.startsWith("javascript:")) js.substring("javascript:".length) else js
        scope.launch {
            try {
